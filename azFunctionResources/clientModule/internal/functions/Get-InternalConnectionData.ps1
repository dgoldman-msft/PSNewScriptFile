...
#>
	...
	process
	{
		try { $uri = '{0}{1}' -f (Get-PSFConfigValue -FullName 'PSNewScriptFile.Client.Uri' -NotNull), $FunctionName }
		catch { $PSCmdlet.ThrowTerminatingError($_) }
		$header = @{ }
		
		#region Authentication
		$unprotectedToken = Get-PSFConfigValue -FullName 'PSNewScriptFile.Client.UnprotectedToken'
		$protectedToken = Get-PSFConfigValue -FullName 'PSNewScriptFile.Client.ProtectedToken'

...
$parentModule = 'PSNewScriptFile'
if (-not $ModuleName) { $ModuleName = 'PSNewScriptFile.Client' }
...
$publishRoot = Join-Path -Path $WorkingDirectory -ChildPath 'publish\PSNewScriptFile'
...
$originalManifestData = Import-PowerShellDataFile -Path "$publishRoot\PSNewScriptFile.psd1"
$prereqHash = @{
